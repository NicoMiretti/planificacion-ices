...
|---|---|---|---|
| **Moderadora (Secretaría Académica)** | Primario | Abre instancias, recibe planificaciones, valida formato, marca tardías, archiva oficiales, hace correcciones leves. **Gestiona el catálogo completo (instituciones, carreras, profesores, materias) desde la interfaz del sistema.** | CU-01 Crear instancia, CU-02 Tablero de revisión, CU-03 Aprobar (parte moderadora), CU-04 Rechazar, CU-05 Aplicar corrección leve, CU-13 Reemplazar oficial, CU-16 Gestionar catálogos |
...
| **Administrador** | Secundario | Superusuario de Django; acceso de emergencia vía `/admin/`. El rol de gestión de catálogos recae en la moderadora en el uso normal. | Cualquier CU |
...
| **Signal de auto-creación** | Soporte | Al asignar carreras a una instancia (evento m2m_changed), crea automáticamente un objeto `Planificacion` por cada materia con titular en la audiencia. | (participa en CU-01) |
